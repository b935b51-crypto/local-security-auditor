...
- `pyproject.toml` is the single package-version source. The 0.9.0 RC wheel reads installed distribution metadata; an uninstalled checkout falls back to the same project file. Build with `uv build`; install the built wheel into a separate environment for release checks. `dist/` is ignored and is not committed by default. See `docs/RELEASE_CANDIDATE.md`.
