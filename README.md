...
Version 1.0.4 is being prepared locally as a finding identity, correlation, and provenance patch. It reduces duplicate presentation, links same-sink behavior signals, and clarifies dependency and AI review status without expanding detector scope. Scans remain offline by default; `--osv` and Gemini `--ai` are separate choices. The default package has no third-party runtime dependencies. Gemini review uses the optional `gemini` extra. Hatchling is a build-only dependency. See [release notes](RELEASE_NOTES.md), [current status](PROJECT_STATUS.md), and the historical [Release Candidate evidence](docs/RELEASE_CANDIDATE.md). See [CLI](docs/CLI.md), [Reporting](docs/REPORTING.md), [AI reviewer](docs/AI_REVIEWER.md), [architecture](docs/ARCHITECTURE.md), [security boundaries](docs/SECURITY_BOUNDARIES.md), and [roadmap](docs/ROADMAP.md).
