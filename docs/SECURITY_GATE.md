...
Hardening #7: valid OSV advisory-reference overflow is incomplete dependency coverage, with fixed diagnostic `DEPENDENCY_OSV_ADVISORY_LIMIT_REACHED`. Gate 1.0 adds `DEPENDENCY_ADVISORY_LIMIT_REACHED` as a `BLOCK` reason even when no Finding was confirmed. Additive JSON 1.1 advisory counters are validated for consistency when present; an overflow report cannot claim `COMPLETE`. Confirmed Findings remain visible and are evaluated normally. This reason describes a local safety processing limit, not a malformed OSV response or 100 confirmed vulnerabilities.

Hardening #8: a file entering bounded large-text Secret mode does not block by size alone. A fully analyzed file contributes `COMPLETE` coverage; unsupported long-line context, decode/read failure, or match/byte/time exhaustion remains `PARTIAL` or `ABORTED` and blocks under the existing coverage policy. The Gate does not infer safety from zero Secret Findings.

Finding identity/correlation hardening leaves policy 1.0 unchanged. Exact duplicates collapse before public counts and remediation; different command/process rules remain distinct machine Findings but one same-sink `finding_group` with CMD as primary and PROCESS as supporting. The Gate evaluates primary roles rather than summing supporting severity. `PARTIAL`, including stale or unavailable dependency intelligence, still blocks. AI `no_eligible_items` and remediation `PATCH_AI_UNAVAILABLE` cannot promote a Gate decision.
