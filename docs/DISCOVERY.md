...
| --- | ---: | ---: | --- |
| `max_file_count` | 100,000 | 250,000 | regular files remaining after intentional scope exclusions |
...
Each `read(size)` is bounded by both per-file byte limits and the remaining total-byte budget. A large admitted file is stat-ed, counted as discovered, then skipped; it is not hashed or fully read. `TraversalStats` distinguishes entries seen, directories visited, files discovered, files admitted, files inspected, bytes inspected, file/directory skips, and intentional file/directory exclusions. Excluded directory children are not enumerated or counted. `max_entries` prevents an enormous single directory from being materialized without bound. Time checks occur between entries/directories, so a single blocking filesystem syscall may exceed the nominal time cap.

## Admission and ignores

The order is: immutable root/reparse/type/entry-budget rules → configurable default traversal excludes → security-scan `exclude` → optional `.gitignore` → explicit `include` override/filter → file-count and file-size limits → prefix read. `include` may restore a file under a default or security exclude but cannot override root safety or hard caps. An excluded directory is pruned unless a trusted include explicitly names that directory or a descendant path. An unrelated include or generic file glob does not reopen caches. Other files must match an include to be admitted when includes exist.

Default scope rules are centralized in `core/scope.py` and supplied through trusted config; they can be overridden by explicit operator config. They prune cache directories (`.uv-cache/`, `.mypy_cache/`, `.pytest_cache/`, `.ruff_cache/`, `.pyright/`, `.pyre/`, `__pycache__/`, `.cache/`, `.tox/`, `.nox/`), environments (`.venv/`, `venv/`, `env/`), dependency vendor trees (`node_modules/`), build output (`dist/`, `build/`, `out/`, `target/`, `.next/`, `.nuxt/`, `.svelte-kit/`, `coverage/`, `htmlcov/`), and VCS metadata (`.git/`, `.hg/`, `.svn/`). Generated `.coverage`, `coverage.xml`, and `*.tsbuildinfo` files are excluded before admission. Built-in file patterns match the basename only; a directory named `example.tsbuildinfo` does not exclude its children. Matching is by path component, not substring: a first-party `my_build_notes.py` remains in scope. Generic `vendor/`, `tests/`, manifests, lockfiles, and `.env` are **not** excluded by default. `respect_gitignore=false` keeps version-control ignore independent from security-scan ignore. The `*.tsbuildinfo` default is Auditor policy whether or not the target has a `.gitignore`; a target `!` negation cannot reopen it. A trusted explicit include can reopen a named file. When enabled, Phase 1 supports only a bounded, root-level `.gitignore` subset: comments, literal names, simple globs, directory suffix `/`, and `!` negation. It does not implement nested `.gitignore`, escaped syntax, bracket classes, or all Git wildmatch rules. Unsupported/oversized/malformed input yields `GITIGNORE_UNAVAILABLE` and a partial result; the target ignore file never grants execution, network, or larger budgets. No Git command is invoked.

`ScopeExclusion` records a root-relative path, class (`CACHE`, `ENVIRONMENT`, `DEPENDENCY_VENDOR`, `BUILD_OUTPUT`, `GENERATED`, or `UNKNOWN`), reason (`EXCLUDED_DEFAULT_*` or `EXCLUDED_USER_POLICY`), and directory/file flag. These are intentional scope omissions, not `SkippedArtifact` failures or findings. Counts cover all exclusions; at most 200 paths are retained and `exclusions_omitted` discloses the remainder. A pruned directory counts once; its children are not inspected merely to count them. This keeps `COMPLETE` meaningful within the declared scan scope. It does not claim safety of excluded content.

Patterns use `/` internally; `\` in patterns is normalized to `/`. A pattern without `/` matches a matching component anywhere, except built-in file exclusions, which match only a file basename; a trailing `/` matches a directory and descendants; a pattern containing `/` matches the relative path using `fnmatch`-style `*` (which may span a separator). Matching is case-insensitive on Windows and case-sensitive elsewhere. Results are sorted by `(casefold(path), path)` for stable order.
...
On 2026-09-24, real Windows synthetic tests under uv-managed Python 3.12.11 created inside-root, outside-root, and looped NTFS junctions inside a disposable temp sandbox. Discovery reported relative `REPARSE_POINT_SKIPPED` paths, `OUTSIDE_ROOT_SKIPPED` for the outside junction, did not admit target content through any junction, rejected a junction selected as scan root, and completed the loop within a hard 10-second worker timeout (without hitting that timeout). Removing the outside junction left its sentinel file intact; all temporary reparse points and sandboxes were removed. A synthetic nested path of approximately 360 characters, including Chinese and spaces, was admitted under bounded file/depth limits. A real temp directory yielded nonzero `st_dev` and `st_ino`; no identity fallback was used there.

Real symlink creation was attempted again but was unavailable due to host privilege/Developer Mode settings. The real symlink integration test is reported as `SKIPPED_ENVIRONMENT_LIMITATION`; simulated reparse and path-boundary regressions remain. No pre-existing safe local UNC share was available, so UNC roots and network filesystem identity were **not validated**. These are verification limits, not claims of universal safety. The path-based TOCTOU residual risk described above remains.
