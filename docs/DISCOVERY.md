...
Default scope rules are centralized in `core/scope.py` and supplied through trusted config; they can be overridden by explicit operator config. They prune cache directories (`.uv-cache/`, `.mypy_cache/`, `.pytest_cache/`, `.ruff_cache/`, `.pyright/`, `.pyre/`, `__pycache__/`, `.cache/`, `.tox/`, `.nox/`), environments (`.venv/`, `venv/`, `env/`), dependency vendor trees (`node_modules/`), build output (`dist/`, `build/`, `out/`, `target/`, `.next/`, `.nuxt/`, `.svelte-kit/`, `coverage/`, `htmlcov/`), and VCS metadata (`.git/`, `.hg/`, `.svn/`). Generated `.coverage` and `coverage.xml` files are excluded. Matching is by path component, not substring: a first-party `my_build_notes.py` remains in scope. Generic `vendor/`, `tests/`, manifests, lockfiles, and `.env` are **not** excluded by default. `respect_gitignore=false` keeps version-control ignore independent from security-scan ignore. When enabled, Phase 1 supports only a bounded, root-level `.gitignore` subset: comments, literal names, simple globs, directory suffix `/`, and `!` negation. It does not implement nested `.gitignore`, escaped syntax, bracket classes, or all Git wildmatch rules. Unsupported/oversized/malformed input yields `GITIGNORE_UNAVAILABLE` and a partial result; the target ignore file never grants execution, network, or larger budgets. No Git command is invoked.
