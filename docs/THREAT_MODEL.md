...
Phase 4 adds malformed manifest/lockfile, include traversal/loop, unresolved-version false-positive, provider schema/size, cache corruption/staleness, and offline-network regression checks. Provider failure cannot be interpreted as "no vulnerabilities." Residual risks include parser CPU/memory use within a bounded file, local cache tampering, provider data quality, and per-request timeout overrun of a best-effort batch deadline. See [Dependency Scanner](DEPENDENCY_SCANNER.md).

Phase 1 adds synthetic tests for boundary checks, mocked reparse behavior, races, Unicode names, large files, and budgets without executing fixture content. Phase 2 adds redaction tests. Phase 3 adds malformed Python, AST node/depth, file/total byte, line/match, source-free evidence, and no-execution regression tests. `ast.parse` is not forcibly interrupted during one parse, and text heuristics can be confused by crafted content; these residual risks are recorded in [SAST](SAST.md) and [Behavior Scanner](BEHAVIOR_SCANNER.md). Real symlink creation was unavailable on the current host; junction, UNC/network identity, and long-path edge cases remain verification gaps documented in [Discovery](DISCOVERY.md). Later phases add adapter, HTML, and terminal regression tests. All fixture keys are clearly fake. No dynamic analysis is authorized by this threat model.
