...
From an installed package: `security-auditor gui`. From this checkout on the verified interpreter: `$env:PYTHONPATH='src'; py -3.14 -m security_auditor gui`. The intended Python 3.12 environment can use `uv run --no-sync --python 3.12 security-auditor gui` once that runtime exists and the package is installed.
