...
Default tests use fake providers and an injected fake SDK client, with no network or real key. The optional `tests/live/test_gemini.py` makes at most one small synthetic request only when `SECURITY_AUDITOR_LIVE_GEMINI_TEST=1` and the existing safe credential loader finds a trusted key in the process environment or tool `.env`; it is otherwise skipped. It uses `medium` thinking and zero retries, checks one request and schema-valid review, and asserts the key is absent from captured output and serialized results. Python 3.12 remains unverified on the current host. SDK/network timeouts are best effort and cannot forcibly preempt a blocking SDK call. Provider data quality, model hallucination, residual prompt injection, and novel secret formats remain review risks. A future Phase 7 reporter must escape and re-redact AI text.
